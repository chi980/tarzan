...
function formatDateString(dateStr: string): string {
  ...
  const month = dateStr.slice(4, 6); // '03'
  const day = dateStr.slice(6, 8); // '01'
  return `${year}/${month}/${day}`; // '25/03/01'
}
...
<template>
  <div class="sale-real-estate-item-container">
    <div class="desc-tag-button-contianer">
      <div>{{ formatDateString(props.data.contractDate) }}</div>
      <div>{{ props.type }}</div>
    </div>
    <div class="building-content">
      ...
      <p id="building-details">
        ...
        <span>{{ props.data.buildingArea }}㎡&nbsp;|&nbsp;</span>
        <span>{{ convertToPyeong(props.data.buildingArea) }}평</span>
      </p>
    </div>
    <p id="building-price">{{ formatToKoreanCurrency(props.data.price) }}</p>
  </div>
</template>
