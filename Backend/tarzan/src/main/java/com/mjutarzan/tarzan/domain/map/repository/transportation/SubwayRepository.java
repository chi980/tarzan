...
public interface SubwayRepository extends JpaRepository<Subway, Long> {
    ...
    @Query("SELECT b FROM Subway b " +
            "WHERE function('ST_DWithin', b.location, function('ST_SetSRID', function('ST_MakePoint', :longitude, :latitude), 4326), :radius) = true")
    List<Subway> findAllWithinRadius(@Param("longitude") double longitude,
                                   @Param("latitude") double latitude,
                                   @Param("radius") double radius);

//    @Query(value = "SELECT * FROM building b WHERE ST_DWithin(b.building_location, ST_SetSRID(ST_MakePoint(:longitude, :latitude), 4326), :radius) = true AND b.building_type = 'subway'", nativeQuery = true)
