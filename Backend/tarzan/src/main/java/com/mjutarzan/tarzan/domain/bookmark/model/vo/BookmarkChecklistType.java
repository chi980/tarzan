...
public enum BookmarkChecklistType {
    ...
    OPTION_ROOM("방 옵션", Arrays.asList("침대", "옷장/북박이장")),
    ...
    CHECK_WINDOW("창문",Arrays.asList("햇빛 잘 들어오는지", "방충망/방범창 이상 없는지", "환기하기에 적합한 크기인지", "옆 건물에서 잘 안 보이는 구조인지")),
    CHECK_BATHROOM("화장실",Arrays.asList("화장실 내부에 창문 있는지","배수구 냄새 안 올라오는지", "샤워 여유 공간 충분한지", "곰팡이 흔적 없는지")),
    CHECK_SURROUNDINGS("주변환경", Arrays.asList("대중교통 이용 편리한지", "주변에 소음 시설 없는지", "편의점, 은행 등 편의시설 있는지", "무인 택배 보관함 있는지", "집 가는 길이 평지인지", "골목이라면 가로등 있는지")),
    CHECK_OPTION("기본 옵션",Arrays.asList("옵션 가구 종류 확인하기", "에어컨/냉장고 작동 점검하기", "화구 종류 체크하기", "옵션 가구 필요 없다면 치워줄 수 있는지 확인하기", "수납 공간 충분한지")),
    CHECK_DETAIL("디테일", Arrays.asList("벽지에 곰팡이 흔적 없는지", "바퀴벌레 약 설치된 흔적 없는지", "콘센트 개수는 충분한지", "인터폰 영상 지원되는지", "옆집 방음 잘 되는지")),
