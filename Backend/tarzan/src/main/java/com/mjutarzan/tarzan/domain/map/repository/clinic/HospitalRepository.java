...
public interface HospitalRepository extends JpaRepository<Hospital, Long> {
    ...
    @Query("SELECT b FROM Hospital b " +
            "WHERE function('ST_DWithin', b.location, function('ST_SetSRID', function('ST_MakePoint', :longitude, :latitude), 4326), :radius) = true")
    List<Hospital> findAllWithinRadius(@Param("longitude") double longitude,
                                   @Param("latitude") double latitude,
                                   @Param("radius") double radius);
//    @Query(value = "SELECT * FROM building b WHERE ST_DWithin(b.building_location, ST_SetSRID(ST_MakePoint(:longitude, :latitude), 4326), :radius) = true AND b.building_type = 'hospital'", nativeQuery = true)
