...
public interface MedicalClinicRepository extends JpaRepository<MedicalClinic, Long> {
    ...
    @Query(value = "SELECT * FROM building b WHERE ST_DWithin(b.location, ST_SetSRID(ST_MakePoint(:longitude, :latitude), 4326), :radius) = true AND b.building_type = medical_clinic", nativeQuery = true)
